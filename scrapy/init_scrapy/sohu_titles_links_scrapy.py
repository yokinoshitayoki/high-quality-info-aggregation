...
def fetch_sohu_titles_and_links(max_scrolls=5, wait_time=2):
    titles = set()
    title_link_dict = {}
        ...
        for card in card_elements:
            try:
                ...
                if title and link and date_str:
                    titles.add(title)
                    title_link_dict[title] = [link, date_str, '搜狐网']
                else:
...
if __name__ == "__main__":
    ...
    titles, title_link_dict = fetch_sohu_titles_and_links(max_scrolls=5, wait_time=2)
    print(f"共抓取到{len(titles)}个标题（去重后）")
    with open('scrapy/sohu_data/sohu_titles.txt', 'w', encoding='utf-8') as f:
        for t in titles:
            f.write(t + '\n')
    print("所有标题已写入 sohu_titles.txt")
    with open('scrapy/sohu_data/sohu_title_links.txt', 'w', encoding='utf-8') as f:
        for title, link in title_link_dict.items():
            f.write(f"{title}\t{link}\n")
    print("标题-链接字典已写入 sohu_title_links.txt")
    ...
    print("标题-链接字典已写入 sohu_title_links.json")