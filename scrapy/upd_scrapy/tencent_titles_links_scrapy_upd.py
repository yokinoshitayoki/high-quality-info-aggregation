...
def load_existing_titles():
    """加载现有的标题列表"""
    existing_titles = set()
    titles_file = 'scrapy/qq_data/qq_titles.txt'
...
def fetch_qq_titles_update(existing_titles, max_scrolls=10, wait_time=1):
    """增量更新腾讯网标题，遇到重复标题时停止"""
    new_titles = []
    new_title_link_dict = {}
        ...
        for card in card_elements:
            ...
            try:
                ...
                if title and link and date_str:
                    ...
                    new_title_link_dict[title] = [link, date_str, '腾讯网']
                    print(f"新增标题: {title}")
                else:
...
if __name__ == "__main__":
    ...
    # 增量爬取新标题
    new_titles, new_title_link_dict = fetch_qq_titles_update(existing_titles, max_scrolls=10, wait_time=1)
    
    print(f"本次更新共抓取到{len(new_titles)}个新标题")
    
    if new_titles:
        # 写入新标题到更新文件
        with open('scrapy/qq_data/qq_title_upd.txt', 'w', encoding='utf-8') as f:
            for t in new_titles:
                f.write(t + '\n')
        print("新标题已写入 qq_title_upd.txt")
        
        # 写入新标题-链接到更新文件
        with open('scrapy/qq_data/qq_title_links_upd.txt', 'w', encoding='utf-8') as f:
            for title, link_info in new_title_link_dict.items():
                f.write(f"{title}\t{link_info[0]}\t{link_info[1]}\t{link_info[2]}\n")
        print("新标题-链接已写入 qq_title_links_upd.txt")
        
        # 写入JSON格式
        with open('scrapy/qq_data/qq_title_links_upd.json', 'w', encoding='utf-8') as f:
            json.dump(new_title_link_dict, f, ensure_ascii=False, indent=2)
        print("新标题-链接-时间字典已写入 qq_title_links_upd.json")
        ...
        with open('scrapy/qq_data/qq_titles.txt', 'w', encoding='utf-8') as f:
            for t in all_titles:
                f.write(t + '\n')
        print("已更新 qq_titles.txt")
        
        # 更新原有链接文件
        all_title_links = {}
        # 先加载原有的
        old_file = 'scrapy/qq_data/qq_title_links.json'
