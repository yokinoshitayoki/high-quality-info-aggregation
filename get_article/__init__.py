...
def load_title_link_data():
    """加载title_link.json文件"""
    try:
        # 读取bind/init_bind/title_link.json文件
        ...
        print(f"成功加载 {len(data)} 条标题-链接数据")
        return data
    
    except Exception as e:
        print(f"加载title_link.json失败: {e}")
        return None
...
def save_to_database(all_results):
    ...
    try:
        ...
        # 找出需要删除的标题（在数据库中但不在成功结果中）
        titles_to_delete = db_titles - success_titles
        
        # 删除失败的数据
        deleted_count = 0
        for title in titles_to_delete:
            try:
                cursor.execute('DELETE FROM title_link WHERE title = ?', (title,))
                if cursor.rowcount > 0:
                    deleted_count += 1
                    print(f"🗑️ 删除失败数据: {title}")
            except Exception as e:
        ...
        print(f"   🗑️ 删除失败: {deleted_count} 篇文章")
        return True
...
def main():
    """主函数：从title_link.json获取数据，然后爬取文章内容"""
    print("开始处理文章内容爬取任务...")
    ...
    print(f"开始并行处理 {len(all_articles)} 篇文章...")
    
    # 使用线程池并行处理所有文章
    all_results = process_article_batch(all_articles)
    
    # 保存结果到数据库
    print(f"总共处理了 {len(all_results)} 篇文章")
    ...
    try:
        ...
        print(f"📄 JSON备份已保存到: {output_file}")
    except Exception as e:
