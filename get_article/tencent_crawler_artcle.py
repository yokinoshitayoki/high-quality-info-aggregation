...
class TencentCrawler:
    ...
    def _extract_content(self, soup):
        """提取文章内容"""
        # 根据腾讯网的文章元素结构，查找带有mpa-paragraph-type="body"属性的<p>标签
        content_paragraphs = []
        
        # 查找所有<p>标签，特别关注有mpa-paragraph-type="body"属性的
        p_tags = soup.find_all('p')
        
        for p in p_tags:
            # 优先处理有mpa-paragraph-type="body"属性的段落
            if p.get('mpa-paragraph-type') == 'body' and self._is_valid_content(p):
                text = p.get_text().strip()
                if text and len(text) > 10:
                    content_paragraphs.append(text)
        
        # 如果没有找到带mpa-paragraph-type="body"的段落，尝试其他<p>标签
    ...
    def _is_valid_content(self, p_tag):
        """判断是否为有效的文章内容"""
        # 检查是否包含导航、广告等关键词
        invalid_keywords = ['广告', '推广', '赞助', '版权', '免责声明', '相关阅读', '腾讯', 'tencent', 'qq']
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("使用方法: python tencent_crawler_artcle.py <URL>")
        print("示例: python tencent_crawler_artcle.py https://news.qq.com/rain/a/20250724A08ZUZ00") 