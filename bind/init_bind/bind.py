...
# 读取AI相关标题
with open('filter/init_filter/ai_titles_v2.txt', 'r', encoding='utf-8') as f:
...
with open('bind/init_bind/title_link.json', 'w', encoding='utf-8') as f:
    json.dump({item['title']: [item['link'], item['date'], item['source']] for item in ai_title_links}, f, ensure_ascii=False, indent=2)
print(f"已写入 bind/init_bind/title_link.json，共{len(ai_title_links)}条")

# 输出到txt文件
with open('bind/init_bind/title_link.txt', 'w', encoding='utf-8') as f:
    for item in ai_title_links:
        f.write(f"{item['title']}\t{item['link']}\t{item['date']}\t{item['source']}\n")
print(f"已写入 bind/init_bind/title_link.txt，共{len(ai_title_links)}条")