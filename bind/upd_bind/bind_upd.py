...
# 读取AI相关标题
with open('filter/upd_filter/ai_titles_v2_upd.txt', 'r', encoding='utf-8') as f:
    titles = [line.strip() for line in f if line.strip()]

# 读取总的标题-链接字典（值为列表：[link, date, source]）
with open('scrapy/all_data/all_title_links_upd.json', 'r', encoding='utf-8') as f:
...
ai_title_links.sort(key=lambda x: parse_date(x['date']), reverse=False)

# 输出到json文件（title: [link, date, source]）
with open('bind/upd_bind/title_link_upd.json', 'w', encoding='utf-8') as f:
    json.dump({item['title']: [item['link'], item['date'], item['source']] for item in ai_title_links}, f, ensure_ascii=False, indent=2)
print(f"已写入 bind/upd_bind/title_link_upd.json，共{len(ai_title_links)}条")

# 输出到txt文件
with open('bind/upd_bind/title_link_upd.txt', 'w', encoding='utf-8') as f:
    for item in ai_title_links:
        f.write(f"{item['title']}\t{item['link']}\t{item['date']}\t{item['source']}\n")
print(f"已写入 bind/upd_bind/title_link_upd.txt，共{len(ai_title_links)}条")